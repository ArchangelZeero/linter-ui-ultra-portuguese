...
class PanelToolbar {
  ...
  render () {
    ...
    if (this.props.errors > 0) filters.push(['error', 'Erros'])
    if (this.props.warnings > 0) filters.push(['warning', 'Avisos'])
    ...
    return (
      <div className='panel-toolbar btn-toolbar'>
        ...
        <button
          ...
          on={{ click: this.props.onMovePrevious }}
        />
        <button
          disabled={disabled}
          className='btn icon icon-arrow-down'
          on={{ click: this.props.onMoveNext }}
        />
