...
export const WORKSPACE_URI = 'atom://linter-ui-ultra'
...
export function visitMessage(message: LinterMessage, reference: boolean = false) {
  ...
  if (reference) {
    if (message.version !== 2) {
      console.warn('[linter-ui-ultra] Only messages v2 are allowed in jump to reference. Ignoring')
      return
    }
    if (!message.reference || !message.reference.file) {
      console.warn('[linter-ui-ultra] Message does not have a valid reference. Ignoring')
      return
    }
...
export function applySolution(textEditor: TextEditor, version: 1 | 2, solution: Object): boolean {
  ...
  if (currentText) {
    ...
    if (currentText !== textInRange) {
      console.warn(
        '[linter-ui-ultra] Not applying fix because text did not match the expected one',
        'expected',
        currentText,
        'but got',
        textInRange,
